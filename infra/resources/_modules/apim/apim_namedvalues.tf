resource "azurerm_api_management_named_value" "io_p_itn_com_rc_func_key" {
  ...
  value               = "@Microsoft.KeyVault(VaultName=${local.key_vault.name};SecretName=rc-func-key)"
  secret              = "true"
}
...
resource "azurerm_api_management_named_value" "io_p_itn_com_services_func_key" {
  ...
  api_management_name = data.azurerm_api_management.apim_itn_api.name
  resource_group_name = data.azurerm_api_management.apim_itn_api.resource_group_name
  value               = "@Microsoft.KeyVault(VaultName=${local.key_vault.name};SecretName=services-func-key)"
  secret              = "true"
}
...
resource "azurerm_api_management_named_value" "io-p-itn-com-pushnotif-func-key" {
  ...
  api_management_name = data.azurerm_api_management.apim_itn_api.name
  resource_group_name = data.azurerm_api_management.apim_itn_api.resource_group_name
  value               = "@Microsoft.KeyVault(VaultName=${local.key_vault.name};SecretName=pushnotif-func-key)"
  secret              = "true"
}
...
resource "azurerm_api_management_named_value" "app_backend_key" {
  ...
  value               = "@Microsoft.KeyVault(VaultName=${var.common_key_vault.name};SecretName=appbackend-APP-BACKEND-PRIMARY-KEY)"
  secret              = true
}
