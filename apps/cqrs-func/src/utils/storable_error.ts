...
export const toStorableError =
  <T>(body: T) =>
  (error: Failure): IStorableError<T> => ({
    body,
...
export const storeAndLogError =
  <T>(
    queueClient: QueueClient,
    telemetryClient: TelemetryClient,
    cqrsLogName: string,
  ) =>
  (processingError: IStorableError<T | unknown>): TE.TaskEither<Error, void> =>
    pipe(
      processingError,
...
export const storeAndLogErrorOrThrow =
  <T>(
    queueClient: QueueClient,
    telemetryClient: TelemetryClient,
    cqrsLogName: string,
