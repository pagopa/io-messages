...
export const aValidService: ValidService = {
  ...
  isVisible: true,
  maxAllowedPaymentAmount: 0 as unknown as WithinRangeInteger<0, 9999999999> &
    number,
...
export const anIncompleteService: {
...
export const aPaymentData = pipe(
  ...
  E.getOrElseW((errors) => {
    throw Error(
      `Malformed Payee in __mocks__: ${errorsToReadableMessages(errors)}`,
    );
  }),
);
