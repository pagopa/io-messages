...
type ServicePreferenceError = CosmosErrors | ISkippedMode | IUnexpectedValue;
...
}) => TaskEither<ServicePreferenceError, readonly BlockedInboxOrChannelEnum[]>;

type ServicePreferencesValues = Omit<
  ServicePreference,
  "accessReadMessageStatus" | "fiscalCode" | "serviceId" | "settingsVersion"
>;
...
const servicePreferenceToBlockedInboxOrChannels: (
...
) => readonly BlockedInboxOrChannelEnum[] = (servicePreference) =>
  /**
  ...
  Object.entries(servicePreference)
    ...
    .filter(
      ([name]) =>
        name in channelToBlockedInboxOrChannelEnum &&
        channelToBlockedInboxOrChannelEnum[
          name as keyof typeof channelToBlockedInboxOrChannelEnum
        ],
    )
...
const getServicePreferenceValueOrError =
  ...
  ({
    ...
    readonly BlockedInboxOrChannelEnum[]
  > =>
    pipe(
...
}) => T.Task<readonly BlockedInboxOrChannelEnum[]>;
