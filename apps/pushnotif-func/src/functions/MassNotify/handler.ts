...
import { MassNotifyBodyLegacy } from "./mass-notify.interface";
...
export const MassNotifyHandler =
  ...
  async (
    logger,
    { subject, body, template, tags },
  ): ReturnType<MassNotifyHandler> => {
    try {
      await sendNotification(subject, body, template, tags);
    } catch (error) {
      logger.error(`Error while sending notification to queue|${error}`);
      return ResponseErrorInternal("Error while sending notification to queue");
    }
...
export const MassNotify = (
  ...
  const middlewaresWrap = withRequestMiddlewares(
    ContextMiddleware(),
    RequiredBodyPayloadMiddleware(MassNotifyBodyLegacy),
  );
