...
public class ReminderServiceTest extends AbstractMock {
  ...
  @MockBean ReminderProducer remProdMock;
  ...
  public void test_getMessageToNotify_producerThrowsJsonProcessingException()
    ...
    Mockito.doThrow(JsonProcessingException.class)
        ...
        .sendReminder(
            ...
            Mockito.any(KafkaTemplate.class),
  ...
  public void test_getMessageToNotify_producerThrowsHttpServerErrorException()
      throws JsonProcessingException {
    List<Reminder> reminders = new ArrayList<>();
    Reminder rem =
        selectReminderMockObject("type", "1", "PAYMENT", "AAABBB77Y66A444A", "123456", 3);
    reminders.add(rem);
    mockGetReadMessageToNotifyWithResponse(reminders);
    mockGetPaidMessageToNotifyWithResponse(new ArrayList<>());
    pagoPAEcommerce409();
  ...
  public void test_getPaymentsByRptid_returnResults() {
    Mockito.when(mockRepository.getPaymentByRptId(Mockito.anyString(), Mockito.anyString()))
        .thenReturn(List.of(new Reminder()));
    List<Reminder> results = reminderService.getPaymentsByRptid("0");
    Assertions.assertTrue(!results.isEmpty());
  }
  ...
  public void test_sendReminderNotification_unreadNotification() {
    ...
    NotificationInfo expected =
        new NotificationInfo()
            .fiscalCode(reminder.getFiscalCode())
            .messageId(reminder.getId())
            .notificationType(NotificationType.REMINDER_READ);
  ...
  @Test
  @DisplayName(
      "it should send a Last Payment reminder notification for PAYMENT messages with due date expiring tomorrow")
  ...
  public void test_sendReminderNotification_notificationNotSent() {
    ...
    Mockito.doThrow(new HttpClientErrorException(HttpStatus.NOT_FOUND))
        .doNothing()
        .when(defaultServiceMessagesApiMock)
        .notify(Mockito.any(NotificationInfo.class));
  ...
  public void test_sendReminderNotification_notificationSend_KO() {
    Reminder reminder =
        selectReminderMockObject("type", "1", "PAYMENT", "AAABBB77Y66A444A", "123456", 3);
    Mockito.doThrow(new HttpClientErrorException(HttpStatus.SERVICE_UNAVAILABLE))
        .doNothing()
        .when(defaultServiceMessagesApiMock)
        .notify(Mockito.any(NotificationInfo.class));
    Assertions.assertThrows(
        HttpClientErrorException.class, () -> reminderService.sendReminderNotification(reminder));
  }
}
