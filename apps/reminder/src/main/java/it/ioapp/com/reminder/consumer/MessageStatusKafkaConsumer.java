...
public class MessageStatusKafkaConsumer {
  ...
  @KafkaListener(
      topics = "${kafka.status}",
      groupId = "message-status-reminder",
      containerFactory = "kafkaListenerContainerFactoryMessStat",
      autoStartup = "${messagestatus.auto.start}")
  public void messageStatusKafkaListener(messageStatus messageStatus) {
    ...
    if (Objects.nonNull(messageStatus)
        && messageStatus.getIsRead() != null
        && messageStatus.getIsRead() == Boolean.TRUE) {
