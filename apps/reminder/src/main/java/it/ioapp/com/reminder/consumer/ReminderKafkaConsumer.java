...
public class ReminderKafkaConsumer {
  ...
  @KafkaListener(
      topics = "${kafka.send}",
      groupId = "reminder-message-send",
      containerFactory = "kafkaListenerContainerFactoryNotify",
      autoStartup = "${messagesend.auto.start}")
  ...
  private void sendNotificationWithRetry(Reminder reminder) {
    ...
    Function<Object, Object> sendNotificationFn =
        Retry.decorateFunction(retry, notObj -> callNotify((Reminder) notObj));
    Retry.EventPublisher publisher = retry.getEventPublisher();
    publisher.onError(
        event -> {
          if (event.getNumberOfRetryAttempts() == attemptsMax) {
            TelemetryCustomEvent.writeTelemetry(
                "ErrorInPostNotification", new HashMap<>(), getErrorMap(reminder, event));
          }
        });
