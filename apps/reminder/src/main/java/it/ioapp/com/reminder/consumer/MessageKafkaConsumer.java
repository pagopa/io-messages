...
public class MessageKafkaConsumer {
  ...
  @KafkaListener(
      ...
      groupId = "reminder-message",
      autoStartup = "${message.auto.start}")
  public void messageKafkaListener(Reminder message) {
    log.info("Received message: {}", message);
    checkNullInMessage(message);
    // boolean shouldSkipThisReminder =
    // Arrays.stream(sendersToSkipDashedString.split("-")).anyMatch(value ->
    // message.getSenderServiceId().equals(value));
    boolean shouldSaveThisMessage =
        Arrays.stream(sendersToUseDashedString.split("-"))
            .anyMatch(value -> message.getSenderServiceId().equals(value));
    if (shouldSaveThisMessage
        && FeatureLevelType.ADVANCED
            .toString()
            .equalsIgnoreCase(message.getFeature_level_type().toString())) {

      if (MessageContentType.PAYMENT
          .toString()
          .equalsIgnoreCase(message.getContent_type().toString())) {
        message.setRptId(
            message
