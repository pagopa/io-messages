...
export class FunctionHost {
  ...
  async start() {
    ...
    child.stdout?.on("data", (chunk: Buffer | string) => {
      ...
      if (lines.length > 0) {
        console.info(`[characterization][func] ${lines}`);
      }
    });
    ...
    child.stderr?.on("data", (chunk: Buffer | string) => {
      ...
      if (lines.length > 0) {
        console.info(`[characterization][func][stderr] ${lines}`);
      }
    });
    ...
    child.once("exit", (code, signal) => {
      if (code !== 0 && code !== null) {
        console.info(
          `[characterization][func] exited with code ${String(code)}`,
        );
      }

      if (signal !== null) {
        console.info(
          `[characterization][func] exited with signal ${String(signal)}`,
        );
      }
    });
