...
/**
 * resolveRCConfiguration returns the remote content configuration for the given
 * configurationID, using the provided cache to avoid fetching the same
 * configuration more than once.
 */
const resolveRCConfiguration = async (
  configurationID: string,
  repository: RCConfigurationRepository,
  cache: Map<string, RCConfiguration>,
): Promise<
  Result<RCConfiguration, GenericError | NotFoundError | TooManyRequestsError>
> => {
  const cached = cache.get(configurationID);
  ...
  cache.set(configurationID, response.value);
...
/**
 * collectSelectedMessages retrieves the metadata/status pairs matching the
 * requested `archived` flag, up to `pageSize + 1` items.
 *
 ...
 */
const collectSelectedMessages = async (
...
): Promise<Result<SelectedMessage[], GenericError | TooManyRequestsError>> => {
  const selectedMessages: SelectedMessage[] = [];
  ...
  do {
    const metadata = await messageMetadataRepository.getMessagesMetadataByUser(
      fiscalCode,
      pageSize * 2, // Buffer.
      cursor,
    ...
    const statuses = await messageStatusRepository.getLatestMessagesStatusByIds(
...
export const makeGetMessagesByUserUseCase =
  ...
  async ({ archived, fiscalCode, maximumID, minimumID, pageSize }) => {
    // To fetch a page of messages, we need to retrieve metadata, statuses and
    // contents. Metadata and statuses collection (including the buffered
    // pagination) is delegated to `collectSelectedMessages`.
    //
    ...
    // operation.
    const selectedMessagesResult = await collectSelectedMessages(
      messageMetadataRepository,
      messageStatusRepository,
      { archived, fiscalCode, maximumID, minimumID, pageSize },
    );

    if (selectedMessagesResult.isErr()) {
      return err(selectedMessagesResult.error);
    }
    ...
    for (const { metadata, status } of selectedMessages) {
      ...
      if (messageContent.third_party_data) {
        ...
        const rcConfiguration = await resolveRCConfiguration(
          configurationID,
          remoteContentConfigurationRepository,
          rcConfigurationCache,
        );

        if (rcConfiguration.isErr()) {
          if (rcConfiguration.error instanceof NotFoundError) {
          ...
          return err(rcConfiguration.error);
        }

        thirdPartyProperties = computeThirdPartyProperties(
          messageContent,
          rcConfiguration.value,
          status.isRead,
        );
      }
