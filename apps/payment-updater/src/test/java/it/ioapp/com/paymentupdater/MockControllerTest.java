...
public class MockControllerTest extends AbstractMock {
  ...
  @Mock private PaymentProducer producer;
  ...
  public void callGetMessagePayment404() throws Exception {
    ...
    MockHttpServletResponse response =
        mvc.perform(get("/api/v1/payment/check/messages/ABC").accept(MediaType.APPLICATION_JSON))
            .andReturn()
            .getResponse();
  ...
  public void callCheckReady() throws Exception {
    // when
    MockHttpServletResponse response =
        mvc.perform(get("/api/v1/health/ready").accept(MediaType.APPLICATION_JSON))
            .andReturn()
            .getResponse();
    // then
    assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());
  ...
  public void callCheckLive() throws Exception {
    // when
    MockHttpServletResponse response =
        mvc.perform(get("/api/v1/health/live").accept(MediaType.APPLICATION_JSON))
            .andReturn()
            .getResponse();
    // then
    assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());
