...
- if Testcontainers inherits a Docker config that points at an unavailable credential helper in a devcontainer or remote workspace, set `DOCKER_CONFIG` to a minimal writable directory **before** launching the Node or Vitest process; some auth paths read Docker config at module initialization, so patching it inside test code can be too late
- for Cosmos or document-store emulators, do not stop at account-level probes such as `getDatabaseAccount()`. Prove the real data-plane path the scenario needs with the SDK, including a write plus point-read or query, and clean any warmup data back out. Some preview or Linux emulators need endpoint discovery disabled or omit metadata on query results
...
- keep recorder and seed helpers independent from target application modules; treat runtime-coupled shared packages the same way, and prefer raw SDK calls, plain JSON payloads, and local characterization schemas over importing app-owned models or decoders
...
These helpers should belong to the characterization folder, not to the target application's runtime modules. Treat shared workspace packages and published internal runtime libraries as equally off-limits here. Avoid importing production model classes, validators, or serialization helpers into them unless there is no credible contract-local alternative and you document that exception plainly.
